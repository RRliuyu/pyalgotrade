...
#######################################################################################################################
        ...
        self.Sma_vol.append(self.__volsma)

        # print(type(round(self.__sma[-1],3)))
        # scatter1.add_xaxis([bar.getDateTime()])
        # scatter1.add_yaxis("vvv", [round(self.__sma[-1],3)])
        ...
        if self.__position is None:
            if bar.getPrice() > self.__pricesma[-1]:
                ...
                time.sleep(0.5)
                shares = int(self.getBroker().getCash() * 0.3 / bars[self.__instrument].getPrice())
                # Enter a buy market order for 10 shares. The order is good till canceled.
                self.__position = self.enterLong(self.__instrument, shares, True)
        # Check if we have to exit the position.
        elif bar.getPrice() < self.__pricesma[-1] and MyStrategy.buyday < bar.getDateTime() and not self.__position.exitActive():
            self.__position.exitMarket()
def run_strategy(smaPeriod):
    ...
    for (root,dirs,files) in os.walk("E:/Stock/Data_Min"):
        for x in range(len(files)):
            #K线图
            df = pd.read_csv("E:/Stock/Data_Min/"+files[x])
            ...
            feed.addBarsFromCSV("fd", "E:/Stock/Data_Min/"+files[x])
            global  myStrategy
            ...
            # plt.savePlot("E:/Stock/html_png_Total/"+files[x]+".png")
            ...
            # kline1.render(path='E:\\Stock/html_png_Total/' + files[x] + '.html')
            df_new = pd.concat([df1,df_new], ignore_index=True)
    df_new.to_csv("E:/Stock/html_png_Total/Total_Min.csv", index=False)
